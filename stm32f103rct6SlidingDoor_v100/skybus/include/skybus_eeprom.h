...
#ifndef __skybus_EEPROM_H
#define __skybus_EEPROM_H
...
typedef struct
{
    /// mang luu gia tri eeprom
    uint16_t data[0x80];
    
    /// write Data 
    uint16_t(*write)(uint16_t VirtAddress, uint16_t Data);
    
    /// read Data
    uint16_t(*read)(uint16_t VirtAddress, uint16_t* Data);
}skybus_eeprom_t;
/* Exported constants --------------------------------------------------------*/
