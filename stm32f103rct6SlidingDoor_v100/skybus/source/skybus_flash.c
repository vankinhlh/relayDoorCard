...
/* Private define ------------------------------------------------------------*/
#define FLASH_START_ADDR   ADDR_FLASH_PAGE_48   /* Start @ of user Flash area */
#define FLASH_END_ADDR     ADDR_FLASH_PAGE_48 + FLASH_PAGE_SIZE   /* End @ of user Flash area */
...
/* Private typedef -----------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static skybus_flash_t* flash;
/* Private function prototypes -----------------------------------------------*/
/* Private functions ---------------------------------------------------------*/

/** @group skybus_FLASH_CONTROL
    @{
*/#ifndef skybus_FLASH_CONTROL
#define skybus_FLASH_CONTROL

HAL_StatusTypeDef skybus_flash_Erase(uint32_t startAddress, uint32_t endAddress)
{
    HAL_FLASH_Unlock();
...
HAL_StatusTypeDef skybus_flash_write(uint32_t address, uint64_t Data)
{
...
uint16_t skybus_flash_read(uint32_t addr)
{
...
{
    skybus_flash_Erase(FLASH_START_ADDR, FLASH_END_ADDR);
    skybus_flash_write(FLASH_START_ADDR, 1234);
    
//    skybus_flash_Erase(FLASH_START_ADDR + 4, FLASH_END_ADDR + 4);
    skybus_flash_write(FLASH_START_ADDR + 4, 5678);
    
    flash->EEPROM_data[0] = skybus_flash_read(FLASH_START_ADDR);
//    flash->EEPROM_data[1] = skybus_flash_read(FLASH_START_ADDR + 4);
    
    skybus_flash_Erase(FLASH_START_ADDR, FLASH_END_ADDR);
    skybus_flash_write(FLASH_START_ADDR, 4321);
    
//    skybus_flash_Erase(FLASH_START_ADDR + 4, FLASH_END_ADDR + 4);
//    skybus_flash_write(FLASH_START_ADDR + 4,  8765);
...
/** @group skybus_FLASH_INIT
    @{
*/#ifndef skybus_FLASH_INIT
#define skybus_FLASH_INIT

/** @brief ham cau hinh flash
    @return dia chi con tro flash
*/
skybus_flash_t* skybus_flash_init(void)
{
    /// cap vung nho cho thu vien
    flash = calloc(sizeof(skybus_flash_t), 1);
    /// gan ham
...
/** @brief ham tra ve dia chi con tro flash
    @return dia chi con tro flash
*/
